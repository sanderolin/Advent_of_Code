...
public class ScienceForHungryPeople01 {

    private static final int TEASPOONS = 100;

    public static void solve() {
        List<Ingredient> ingredients = readFromFile();
        long scoreOfHighestScoringCookie = findHighestScoreRecursive(ingredients, new int[ingredients.size()], 0, TEASPOONS);
        System.out.println(scoreOfHighestScoringCookie);
    }

    private static long findHighestScoreRecursive(List<Ingredient> ingredients, int[] teaspoons, int index, int remainingTeaspoons) {
        ...
        for (int i = 0; i <= remainingTeaspoons; i++) {
            teaspoons[index] = i;
            highestScore = Math.max(highestScore, findHighestScoreRecursive(ingredients, teaspoons, index + 1, remainingTeaspoons - i));
        }
        return highestScore;
    }
